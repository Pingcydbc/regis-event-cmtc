...
<head>
    ...
    <style>
        ...
        .card-shadow { box-shadow: 0 4px 15px -2px rgba(0, 0, 0, 0.04), 0 2px 8px -2px rgba(0, 0, 0, 0.02); }
        .nav-gradient { background: linear-gradient(135deg, #991b1b 0%, #7f1d1d 100%); }
...
<body class="bg-[#f8fafc] min-h-screen pb-8">
        ...
        <div class="max-w-7xl mx-auto px-4 py-2 flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div class="hidden sm:block">
                    ...
                    <p class="text-[9px] text-red-200 uppercase tracking-widest mt-0.5 font-bold">Chiang Mai Technical College</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <div id="refresh_timer" class="hidden md:flex items-center bg-black/10 px-3 py-1.5 rounded-xl text-[9px] font-black gap-2 border border-white/5 backdrop-blur-sm">
                    <span class="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></span>
                    รีเฟรช: <span id="timer_sec" class="w-3 inline-block">30</span>วินาที
                </div>
                <button onclick="switchView('stats')" id="nav_stats" class="bg-white/10 hover:bg-white/20 text-[10px] px-4 py-2 rounded-xl transition-all font-black border border-white/10 hidden btn-hover">แดชบอร์ด</button>
                <button onclick="switchView('chats')" id="nav_chats" class="bg-white text-red-900 text-[10px] px-4 py-2 rounded-xl transition-all font-black border border-white shadow-lg flex items-center gap-2 btn-hover">
                    ศูนย์รับเรื่องแชท
    ...
    <div id="stats_view" class="max-w-6xl mx-auto px-4 py-6 space-y-6 animate-fade-in">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-slate-200 pb-4">
            ...
            <div class="bg-white px-4 py-2.5 rounded-2xl card-shadow border border-slate-100 flex items-center gap-4">
                <div class="text-right">
                    ...
                    <h4 class="text-xl font-black text-blue-600 tracking-tighter leading-none"><span id="all_reg_label">0/0</span></h4>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Filter Card -->
            <div class="bg-white p-6 rounded-[2rem] card-shadow border border-slate-50 space-y-6 relative overflow-hidden">
                ...
                <h3 class="font-black text-lg text-slate-800 flex items-center gap-3 relative z-10">
                    <div class="w-10 h-10 bg-red-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-red-100">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" stroke-width="2.5"></path></svg>
                    </div>
                    ตัวกรองข้อมูล
                </h3>
                <div class="space-y-4 relative z-10">
                    <div class="group">
                        <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 px-1 transition-colors group-focus-within:text-red-600">แผนกวิชา (Department)</label>
                        <select id="filter_dept" onchange="loadGroupsByDept()" class="w-full bg-slate-50 border-2 border-slate-100 text-slate-700 text-xs font-bold rounded-xl p-3 outline-none focus:ring-4 focus:ring-red-50/50 focus:border-red-600 transition-all cursor-pointer">
                            <option value="">ทุกแผนกวิชา</option>
                        </select>
                    </div>
                    <div class="group">
                        <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 px-1 transition-colors group-focus-within:text-red-600">กลุ่มเรียน (Class Group)</label>
                        <select id="filter_group" onchange="updateStats()" class="w-full bg-slate-50 border-2 border-slate-100 text-slate-700 text-xs font-bold rounded-xl p-3 outline-none focus:ring-4 focus:ring-red-50/50 focus:border-red-600 transition-all cursor-pointer">
            ...
            <!-- Stats Card -->
            <div class="lg:col-span-2 bg-white p-6 rounded-[2rem] card-shadow border border-slate-50 grid grid-cols-1 md:grid-cols-2 gap-8 items-center relative overflow-hidden">
                <div class="relative h-[180px] flex items-center justify-center">
                    <canvas id="regChart"></canvas>
                    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <span id="stat_percent" class="text-3xl font-black text-slate-800 leading-none tracking-tighter">0%</span>
                        <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">อัตราสำเร็จ</span>
                    </div>
                </div>
                <div class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-emerald-50/50 p-4 rounded-[1.5rem] border border-emerald-100/50 shadow-inner group">
                            <p class="text-[8px] font-black text-emerald-600 uppercase tracking-widest mb-1">ลงทะเบียนแล้ว</p>
                            <h4 class="text-2xl font-black text-emerald-700 tracking-tighter group-hover:scale-105 transition-transform" id="stat_registered">0</h4>
                            <span class="text-[8px] font-black text-emerald-400 uppercase tracking-widest mt-0.5 block">คน (Persons)</span>
                        </div>
                        <div class="bg-slate-50/50 p-4 rounded-[1.5rem] border border-slate-100/50 shadow-inner group">
                            <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">เป้าหมายทั้งหมด</p>
                            <h4 class="text-2xl font-black text-slate-800 tracking-tighter group-hover:scale-105 transition-transform" id="stat_total">0</h4>
                            <span class="text-[8px] font-black text-slate-300 uppercase tracking-widest mt-0.5 block">คน (Total)</span>
                        </div>
                    </div>
                    <div class="bg-slate-900 p-5 rounded-[1.5rem] shadow-xl shadow-slate-900/10 relative overflow-hidden">
                        <div class="absolute top-0 right-0 w-24 h-24 bg-white/5 rounded-full -mr-12 -mt-12"></div>
                        <div class="flex justify-between items-end mb-2 relative z-10">
                            <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest">ความคืบหน้าภาพรวม</span>
                        ...
                        <div class="w-full bg-white/10 h-2 rounded-full overflow-hidden relative z-10">
                            <div id="all_progress_bar" class="bg-gradient-to-r from-red-600 to-red-400 h-full rounded-full transition-all duration-1000" style="width: 0%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white p-6 rounded-[2rem] card-shadow border border-slate-50 space-y-6">
            <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div class="relative w-full md:w-[28rem] group">
                    <input type="text" id="searchInput" placeholder="พิมพ์ชื่อ หรือ รหัสประจำตัว..." class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-6 py-4 pr-14 outline-none focus:ring-8 focus:ring-slate-50 focus:border-slate-900 transition-all font-black text-slate-700 text-sm placeholder:text-slate-300">
                    <div class="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-slate-900 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-width="2.5"></path></svg>
                    </div>
                </div>
            </div>
            
            <div id="searchResults" class="hidden overflow-hidden rounded-2xl border border-slate-100 shadow-inner bg-slate-50/50">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="searchResultsBody" class="divide-y divide-slate-100 bg-white font-bold text-slate-700 text-xs">
                            <!-- Results go here -->
                        </tbody>
                    </table>
                </div>
            ...
            <div class="bg-white p-6 rounded-[2rem] card-shadow border border-slate-50 space-y-6 relative overflow-hidden">
                <h3 class="font-black text-lg text-slate-800 flex items-center gap-3">
                    <div class="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-100">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" stroke-width="2.5"></path></svg>
                    </div>
                    นำเข้าข้อมูล (CSV)
                </h3>
                <form id="importForm" class="flex flex-col gap-4">
                    <div class="relative group">
                        <input type="file" name="excel_file" accept=".csv" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                        <div class="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-6 text-center group-hover:border-blue-500 group-hover:bg-blue-50/30 transition-all">
                            <div class="flex flex-col items-center gap-1.5">
                                <svg class="w-6 h-6 text-slate-300 group-hover:text-blue-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" stroke-width="2"></path></svg>
                                <span class="text-[9px] font-black text-slate-400 group-hover:text-blue-600 uppercase tracking-widest">คลิกเพื่อเลือกไฟล์ .CSV</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex gap-3">
                        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-xl transition-all shadow-lg text-[10px] uppercase tracking-widest btn-hover">นำเข้าและประมวลผล</button>
                        <a href="process_admin.php?action=download_template" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-black px-6 py-4 rounded-xl transition-all text-[10px] uppercase tracking-widest flex items-center justify-center btn-hover">เทมเพลต</a>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white p-6 rounded-[2rem] card-shadow border border-slate-50 space-y-6 relative overflow-hidden">
                <h3 class="font-black text-lg text-slate-800 flex items-center gap-3">
                    <div class="w-10 h-10 bg-emerald-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-emerald-100">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" stroke-width="2.5"></path></svg>
                    </div>
                    ส่งออกรายงาน
                </h3>
                <div class="grid grid-cols-2 gap-4">
                    <button onclick="exportData('pdf')" class="bg-slate-900 hover:bg-black text-white font-black py-5 rounded-xl transition-all shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 btn-hover">
                    ...
                    <button onclick="exportData('excel')" class="bg-emerald-600 hover:bg-emerald-700 text-white font-black py-5 rounded-xl transition-all shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 btn-hover">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M6 2a2 2 0 00-2 2v12a2 2 0 002 2h8a2 2 0 002-2V7.414A2 2 0 0015.414 6L12 2.586A2 2 0 0010.586 2H6zm2 10a1 1 0 10-2 0v3a1 1 0 102 0v-3zm2-3a1 1 0 011 1v5a1 1 0 11-2 0v-5a1 1 0 011-1zm4-1a1 1 0 10-2 0v7a1 1 0 102 0V8z" clip-rule="evenodd"></path></svg>
                        ไฟล์ EXCEL
                    </button>
                </div>
            </div>
    ...
    <div id="chats_view" class="hidden max-w-7xl mx-auto px-4 py-6 h-[calc(100vh-100px)] min-h-[500px] animate-fade-in">
        <div class="bg-white rounded-[2rem] shadow-2xl border border-slate-100 flex h-full overflow-hidden relative glass-effect card-shadow">
            ...
            <div id="chat_sidebar" class="w-full sm:w-[300px] border-r border-slate-100 flex flex-col bg-slate-50/30 transition-all duration-300">
                ...
                <div id="chat_user_list" class="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
                    <div class="py-16 text-center text-slate-300 font-black italic text-[9px] uppercase tracking-widest">กำลังโหลดรายชื่อ...</div>
                </div>
            </div>

            <!-- Chat Window -->
            <div id="chat_main" class="absolute inset-0 z-10 bg-white flex flex-col translate-x-full sm:relative sm:translate-x-0 sm:flex-1 transition-transform duration-500">
                <div id="chat_header" class="p-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0 shadow-sm relative z-10">
                    <div class="flex items-center gap-4">
                        <button onclick="closeChatMobile()" class="sm:hidden p-3 bg-slate-50 hover:bg-slate-100 rounded-xl transition text-slate-500 shadow-inner">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7" stroke-width="3"></path></svg>
                        </button>
                        <div class="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-300 shadow-inner border border-slate-100">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path></svg>
                        </div>
                        <div class="min-w-0">
                            <h4 id="active_user_name" class="font-black text-slate-800 text-base truncate tracking-tight">เลือกบทสนทนา</h4>
                            <div class="flex items-center gap-1.5">
                                <span class="w-1.5 h-1.5 bg-slate-200 rounded-full" id="status_dot"></span>
                                <p id="active_user_status" class="text-[8px] text-slate-400 uppercase tracking-widest font-black">แสตนด์บาย</p>
                            </div>
                        </div>
                    </div>
                    <button onclick="loadChatUsers()" class="p-3 bg-slate-50 hover:bg-slate-100 rounded-xl transition text-slate-400 group">
                        <svg class="w-4 h-4 group-hover:rotate-180 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" stroke-width="2.5"></path></svg>
                    </button>
                </div>
                ...
                <div id="admin_chat_body" class="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50 custom-scrollbar">
                    <div class="h-full flex flex-col items-center justify-center text-slate-200 space-y-6">
                        <div class="w-24 h-24 bg-white rounded-[2rem] shadow-2xl border border-slate-50 flex items-center justify-center">
                            <svg class="w-12 h-12 text-slate-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" stroke-width="1.5"></path></svg>
                        </div>
    ...
    <script>
        ...
        async function loadGroupsByDept() {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success) updateGroupDropdown(data.groups);
            } catch (e) { console.error(e); }
            updateStats();
        }
        ...
        async function loadChatUsers() {
            try {
                ...
                const text = await res.text(); if (text.includes('<html>')) return;
                const data = JSON.parse(text);
                if (data.success) {
        ...
        document.getElementById('admin_chat_form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if ((await res.json()).success) fetchAdminMessages();
            } catch (e) {}
        });
        ...
        document.getElementById('importForm').addEventListener('submit', async (e) => {
            ...
            try {
                const res = await (await fetch('process_admin.php?action=import_data', { method: 'POST', body: fd })).json();
                Swal.fire({ icon: res.success ? 'success' : 'error', title: res.success ? 'สำเร็จ' : 'ผิดพลาด', text: res.message }).then(() => { if(res.success) window.location.reload(); });
