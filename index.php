...
<head>
    ...
    <style>
        ...
        .card-shadow {
            box-shadow: 0 10px 30px -10px rgba(0, 0, 0, 0.08), 0 4px 15px -8px rgba(0, 0, 0, 0.04);
        }
...
<body class="bg-[#f8fafc] flex flex-col items-center justify-center min-h-screen p-4 space-y-6">
    ...
    <div
        class="bg-white p-8 rounded-[2rem] card-shadow w-full max-w-sm border-t-[8px] border-red-700 transform transition-all relative overflow-hidden animate-fade-in">
        ...
        <div class="flex flex-col items-center mb-8 relative z-10">
            <div class="mb-4">
                <img src="logo.png" alt="โลโก้วิทยาลัยเทคนิคเชียงใหม่"
        ...
        <form id="checkForm" class="space-y-6 relative z-10">
            <div class="space-y-2">
                <label
                    class="block text-[9px] font-black text-slate-400 text-center uppercase tracking-widest">กรอกเลขประจำตัวประชาชน
                    (นักเรียน นักศึกษา)</label>
                <input type="text" id="id_card" name="id_card" placeholder="เช่น 150996600001" required
                    autocomplete="off" oninput="this.value = this.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();"
                    class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-center text-xl font-black tracking-widest focus:ring-8 focus:ring-red-50 focus:border-red-600 outline-none transition-all duration-300 text-slate-800 placeholder:text-slate-200">
            </div>

            <button type="submit"
                class="w-full bg-red-700 hover:bg-red-800 text-white font-black py-4 rounded-2xl transition-all duration-300 shadow-xl shadow-red-700/10 text-xs uppercase tracking-widest flex items-center justify-center gap-3 btn-hover">
                ยืนยันข้อมูล
        ...
        <div class="mt-8 text-center border-t border-slate-50 pt-5">
            <span class="text-[8px] text-slate-300 tracking-widest uppercase font-black">Chiang Mai Technical
                College</span>
        </div>
    </div>
    ...
    <a href="?login=1"
        class="text-slate-300 hover:text-slate-500 text-[9px] transition font-black uppercase tracking-widest opacity-50 hover:opacity-100">จัดการระบบ</a>
    ...
    <div id="chat_widget" class="fixed bottom-4 right-4 z-[100] flex flex-col items-end gap-4 max-w-[calc(100vw-2rem)]">
        <!-- Chat Window -->
        <div id="chat_window"
            class="hidden w-[300px] sm:w-[350px] h-[480px] max-h-[75vh] bg-white rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border border-slate-100 transition-all duration-500 transform translate-y-4 opacity-0">
            <div class="bg-red-700 p-5 text-white flex justify-between items-center shadow-lg shrink-0">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center backdrop-blur-md shadow-inner">
                ...
                <div class="flex items-center gap-2">
                    <!-- ปุ่มย่อแชท -->
                    <button onclick="toggleChat()" class="bg-white/5 hover:bg-white/10 p-2 rounded-lg transition-all"
                        title="ย่อหน้าต่าง">
                    ...
                    <!-- ปุ่มจบแชท (กากบาท) -->
                    <button onclick="endChat()" class="bg-white/5 hover:bg-white/10 p-2 rounded-lg transition-all"
                        title="จบการสนทนาและลบข้อมูล">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
            ...
            <div id="chat_body" class="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50/50 custom-scrollbar">
                <div class="text-center py-16 text-slate-300 text-[9px] font-black uppercase tracking-widest italic">
                    กำลังเริ่มการสนทนา...</div>
            </div>

            <form id="chat_form" class="p-4 bg-white border-t border-slate-100 flex gap-2 shrink-0 items-center">
                <input type="text" id="chat_input" placeholder="พิมพ์ข้อความที่นี่..." autocomplete="off"
                    class="flex-1 bg-slate-50 border-2 border-slate-100 rounded-xl px-5 py-3 text-xs font-bold focus:ring-4 focus:ring-red-50 focus:border-red-600 outline-none transition-all">
                <button type="submit"
                    class="bg-red-700 text-white w-10 h-10 rounded-xl hover:bg-red-800 transition-all shadow-lg flex items-center justify-center shrink-0 btn-hover group">
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="h-5 w-5 transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform"
        ...
        <!-- Floating Button -->
        <button id="chat_toggle_btn" onclick="toggleChat()"
            class="bg-red-600 hover:bg-red-700 text-white font-black p-3 px-5 rounded-full shadow-xl transition-all duration-300 transform hover:scale-105 flex items-center gap-3 border border-white/10">
            ...
            <span id="chat_badge"
                class="hidden absolute -top-1 -right-1 w-4 h-4 bg-red-800 rounded-full border-2 border-white animate-pulse"></span>
        </button>
    </div>
    ...
    <div id="modal" class="fixed inset-0 bg-slate-900/60 hidden items-center justify-center p-4 backdrop-blur-md z-50">
        <div
            class="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-sm space-y-6 transform transition-all duration-500 border-t-[10px] border-red-700 animate-fade-in">
            <h3
                class="text-lg font-black text-slate-800 pb-3 text-center border-b border-slate-50 uppercase tracking-tight flex items-center justify-center gap-2.5">
            ...
            <div class="space-y-3.5 text-xs font-bold border-b border-slate-50 pb-6">
                <div class="flex items-center justify-between bg-slate-50 p-3.5 rounded-2xl">
                    ...
                    <span id="modal_id_card"
                        class="text-slate-800 tracking-widest font-black text-sm">1234567890123</span>
                </div>
                <div class="space-y-2.5 px-1.5">
                    <div class="flex justify-between items-center gap-4">
                        ...
                        <span id="modal_student_name" class="text-slate-800 font-black text-right truncate">นายสมชาย
                            แสนดี</span>
                    </div>
                    <div class="flex justify-between items-center gap-4">
                    ...
                    <div class="flex justify-between items-center gap-4">
                        ...
                        <span id="modal_department"
                            class="text-red-700 font-black text-right truncate">เทคโนโลยีสารสนเทศ</span>
                    </div>
                </div>
            </div>
            ...
            <form id="registerForm" class="space-y-5">
                ...
                <div class="group">
                    <label
                        class="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1 transition-colors group-focus-within:text-red-600">ชื่อ-นามสกุล
                        ผู้ปกครอง (แก้ไขได้)</label>
                    <input type="text" id="modal_parent_name" name="modal_parent_name" required autocomplete="off"
                        class="w-full px-5 py-3.5 bg-slate-50 border-2 border-slate-100 rounded-xl font-black focus:ring-4 focus:ring-red-50 focus:border-red-600 outline-none transition-all text-slate-800 text-sm">
                </div>

                <div class="flex gap-3 pt-2">
                    <button type="button" onclick="closeModal()"
                        class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-400 font-black py-4 rounded-xl transition-all text-[9px] uppercase tracking-widest btn-hover">ยกเลิก</button>
                    <button type="submit"
                        class="flex-[2] bg-red-700 hover:bg-red-800 text-white font-black py-4 rounded-xl transition-all shadow-lg text-[9px] uppercase tracking-widest btn-hover">ยืนยันลงทะเบียน</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="loginModal"
        class="fixed inset-0 bg-slate-900/80 <?php echo $show_login ? 'flex' : 'hidden'; ?> items-center justify-center p-4 backdrop-blur-xl z-[60]">
        <div
            class="bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-sm border-t-[10px] border-slate-900 relative transform transition-all animate-fade-in">
            <button onclick="closeLoginModal()"
                class="absolute top-6 right-6 text-slate-300 hover:text-slate-900 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
            ...
            <div class="flex flex-col items-center mb-8">
                ...
                <p class="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-1.5 italic">Authorized
                    Access Only</p>
            </div>
            <?php if ($login_error): ?>
                <div
                    class="bg-red-50 text-red-600 p-3 rounded-xl text-[9px] mb-6 text-center border border-red-100 font-black uppercase tracking-widest animate-pulse">
            ...
            <form method="POST" class="space-y-5">
                ...
                <div class="space-y-1.5">
                    <label
                        class="block text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Username</label>
                    <input type="text" name="username" required
                        class="w-full px-5 py-3.5 bg-slate-50 border-2 border-slate-100 rounded-xl font-black focus:ring-4 focus:ring-slate-100 focus:border-slate-900 outline-none transition-all text-sm">
                </div>
                <div class="space-y-1.5">
                    <label
                        class="block text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Password</label>
                    <input type="password" name="password" required
                        class="w-full px-5 py-3.5 bg-slate-50 border-2 border-slate-100 rounded-xl font-black focus:ring-4 focus:ring-slate-100 focus:border-slate-900 outline-none transition-all text-sm">
                </div>
                <button type="submit"
                    class="w-full bg-slate-900 hover:bg-black text-white font-black py-4 rounded-xl transition-all shadow-lg mt-4 uppercase tracking-widest text-[10px] btn-hover">เข้าสู่ระบบ</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        async function handleResponse(response, isSilent = false) {
        ...
        async function fetchMessages() {
            if (document.getElementById('chat_window').classList.contains('hidden')) return;
            try {
        ...
        async function showFixStudentModal() {
            ...
            try {
                ...
                const data = await handleResponse(res);
                if (data && !data.success) { Swal.fire('ผิดพลาด', 'โหลดข้อมูลไม่สำเร็จ', 'error'); return; }
                if (data) {
                        ...
                        didOpen: () => {
                            ...
                            dSel.addEventListener('change', async () => {
                                ...
                                try {
                                    ...
                                    const gData = await handleResponse(gRes);
                                    if (gData) {
        ...
        async function saveFixStudent(data) {
            ...
            try {
                ...
                const rd = await handleResponse(res);
                if (rd) {
